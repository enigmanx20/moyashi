% Initial settings
function PCA_QDA(mozmin, mozmax, pcx, pcy)
...
cqs = ClassificationDiscriminant.fit([PC1, PC2], GROUP, 'DiscrimType','quadratic');
K = cqs.Coeffs(1,2).Const;
L = cqs.Coeffs(1,2).Linear; 
Q = cqs.Coeffs(1,2).Quadratic;
f = @(PC1,PC2) K + L(1)*PC1 + L(2)*PC2 + Q(1,1)*PC1.^2 +(Q(1,2)+Q(2,1))*PC1.*PC2...
    + Q(2,2)*PC2.^2;
...
title('{\bf Quadratic Classification}', 'Color', [0.6 0 0.6])
% gname
hold off;
